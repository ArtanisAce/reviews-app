...
exports.calculateGeneral = (data) => (data.reduce((acc, act) => acc + (act.ratings.general.general * weightValue(act.entryDate)), 0) / data.length).toFixed(2);
...
exports.calculateAspects = (data) => {
    ...
    if (data) {
        ...
        aspects.forEach((aspect) => {
            aspectsObject[aspect] = (data.reduce((acc, act) => acc + (act.ratings.aspects[aspect] * weightValue(act.entryDate)), 0) / data.length).toFixed(2);
        });
    }
